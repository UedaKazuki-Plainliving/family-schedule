...
public class MemberController {
    ...
    @PostMapping
    public ResponseEntity<MemberResponse> create(@RequestBody MemberRequest req) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.create(req));
    }

    @PutMapping("/{id}")
    public MemberResponse rename(@PathVariable Integer id, @RequestBody MemberRequest req) {
