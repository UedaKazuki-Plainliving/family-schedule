...
| ST-04-08 | FR-12: インライン編集では「内容」のみ変更可能（誰が・日付のUIは非表示） | S-04 | H |
...
| ST-CB-01 | NFR-05: Microsoft Edge（最新版）で基本操作が動作する | 全画面 | M |
| ST-CB-02 | NFR-05: Safari（最新版）で基本操作が動作する | 全画面 | M |
...
| 6 | トーストメッセージを確認する（FR-16） | 「保存しました」のトーストが画面下部に表示される |
...
| 確認項目 | 合格 | 不合格 |
|---------|------|--------|
| モーダルが閉じる | 保存後にモーダルが消える | モーダルが残る |
| グリッドに反映 | 「在宅勤務」がグリッドに表示される | 表示されない |
| トーストメッセージ（FR-16） | 「保存しました」が表示される | 何も表示されない |
...
| 5 | トーストメッセージを確認する（FR-16） | 「更新しました」が表示される |
...
| 確認項目 | 合格 | 不合格 |
|---------|------|--------|
| セルの内容更新 | 「部活（試合）」に変わっている | 変わっていない |
| トーストメッセージ（FR-16） | 「更新しました」が表示される | 何も表示されない |
...
#### ST-04-08 インライン編集では「内容」のみ変更できること（FR-12）
...
**確認内容:** インライン編集モードでは「内容（テキスト）」だけが変更可能で、「誰が（memberId）」と「日付（date）」を変更するUIが表示されないこと。

UI操作手順:

| ステップ | 操作 | 期待結果 |
|---------|------|---------|
| 1 | 長女の「部活」をタップしてインライン編集モードに入る | textarea（`.schedule-item-input`）が表示される |
| 2 | 画面を確認する | テキスト入力欄**のみ**表示されており、「誰が」選択ボタンや日付変更ボタンは**表示されていない** |
| 3 | textarea に「部活（修正）」と入力して Enter で保存する | 内容だけが変更され、memberId や date は変わらない |
| 4 | グリッドの表示を確認する | 長女の今日の予定が「部活（修正）」に変わっている（他の人の予定は影響なし） |

`snap("ST-04-08_インライン編集は内容のみ")`

**合否判定:**

| 確認項目 | 合格 | 不合格 |
|---------|------|--------|
| 「誰が」ボタンの非表示 | インライン編集中に「誰が」切替ボタンが表示されていない | 表示されている |
| 日付変更UIの非表示 | 日付変更フィールドが表示されていない | 表示されている |
| 内容の変更 | テキストだけ更新される | memberId や date が変わる |

Playwright操作例:
```javascript
test('ST-04-08: インライン編集は内容のみ変更可能（FR-12）', async ({ page }) => {
  await page.goto('/');
  await setCurrentUser(page, { id: 3, name: '長女', displayOrder: 3 });
  await page.reload();
  // インライン編集開始
  ...
  const editInput = page.locator('.schedule-item-input').first();
  await expect(editInput).toBeVisible();
  // 「誰が」切替ボタンが表示されていないことを確認（FR-12）
  await expect(page.locator('#who-btns')).toBeHidden();
  // 日付変更UIが表示されていないことを確認（FR-12）
  await expect(page.locator('#date-input')).toBeHidden();
  // 内容だけ変更できることを確認
  await editInput.fill('部活（修正）');
  await editInput.press('Enter');
  await page.waitForTimeout(500);
  await expect(page.locator('.schedule-item-text', { hasText: '部活（修正）' })).toBeVisible();
  await snap(page, 'ST-04-08_インライン編集は内容のみ');
});
```
...
> **対応要件：** NFR-05（Chrome / Safari / Edge 最新2バージョンで動作）
>
> Chromeでの動作は第4・5章の全テストケースでカバー済みです。本章では **Edge** と **Safari** での
> 最低限の動作確認（スモークテスト）を行います。

---

### ST-CB-01 Microsoft Edgeで基本操作が動作すること
...
| 前提条件 | Docker Compose 起動済み（`docker compose up -d`）。Edgeがインストール済み |
| 優先度 | M |

**Edgeのインストール（まだ入っていない場合）:**

Windows 11 には標準でインストールされています。アドレスバーに `edge://settings/help` と入力して
バージョンを確認し、最新版に更新してください。

**UI操作手順:**

| ステップ | 操作 | 期待結果 |
|---------|------|---------|
| 1 | Edgeを起動し `http://localhost:8080` を開く | S-01（利用者選択画面）が表示される。レイアウト崩れがない |
| 2 | 「お父さん」ボタンをクリックする | S-02（スケジュール画面）に遷移する。今日・明日の2カラムが表示される |
| 3 | ＋ボタンをクリックして予定を1件追加する（内容：「Edge動作確認」） | 予定が追加され、グリッドに表示される |
| 4 | 追加した予定テキストをクリックしてインライン編集する（内容変更 → Enter） | 内容が更新される |
| 5 | ✕ボタンで予定を削除する | 予定が削除され、UNDOトーストが表示される |
| 6 | F12を押してコンソールエラーを確認する | `[ERROR]` や `Uncaught` が出ていない |

`snap("ST-CB-01_Edge基本動作確認")`

**合否判定:**

| 確認項目 | 合格 | 不合格 |
|---------|------|--------|
| 画面表示 | レイアウト崩れなし、全テキストが読める | ボタンやグリッドが重なる・消える |
| CRUD操作 | 追加・編集・削除が全て完了する | いずれかのステップでエラーになる |
| コンソールエラー | エラーなし（または既知のwarningのみ） | `[ERROR]` / `Uncaught` が出る |

---

### ST-CB-02 Safariで基本操作が動作すること

| 項目 | 内容 |
|------|------|
| テストケースID | ST-CB-02 |
| 非機能要件 | NFR-05 |
| 対象ブラウザ | Safari 最新版（macOS または iOS） |
| 前提条件 | サーバー起動済み。SafariからアクセスできるIPアドレスまたはホスト名でサーバーにアクセス可能 |
| 優先度 | M |

> **アクセス方法の確認：**
> Safariが同一PC（Mac）にある場合は `http://localhost:8080` でアクセス可能です。
> iPhoneなど別デバイスからの場合は、PCのローカルIPアドレス（例: `http://192.168.1.10:8080`）を使用してください。
> PCのIPは `ipconfig`（Windows）または `ifconfig`（Mac）で確認できます。

**UI操作手順（スマホ（iOS Safari）の場合はタップ操作で実施）:**

| ステップ | 操作 | 期待結果 |
|---------|------|---------|
| 1 | Safariで対象URLを開く | S-01（利用者選択画面）が表示される。ボタンが全員分表示されている |
| 2 | 「お父さん」をタップ（またはクリック） | S-02が表示される。今日・明日の2カラムが表示される |
| 3 | ＋ボタンをタップして予定を追加する（内容：「Safari動作確認」） | 予定が追加され、グリッドに表示される |
| 4 | iOSの場合：画面を左右にフリックする | 日付が1日ずれる（フリックナビゲーションが動作する） |
| 5 | 追加した予定の✕ボタンで削除する | 予定が削除され、UNDOトーストが表示される |
| 6 | Safariの開発ツールでコンソールエラーを確認する（任意） | 致命的なエラーが出ていない |

`snap("ST-CB-02_Safari基本動作確認")`

**合否判定:**

| 確認項目 | 合格 | 不合格 |
|---------|------|--------|
| 画面表示 | レイアウト崩れなし、ボタン・テキストが読める | 画面が崩れる・要素が重なる |
| タッチ操作 | タップでボタン選択・予定追加が完了する | タップが反応しない・フォームが開かない |
| フリック（iOSの場合） | 左右スワイプで日付が移動する | フリックが無反応 |
| CRUD操作 | 追加・削除が完了する | エラーが表示される |

---

## 7. 付録: よくある失敗と対処法

### 7.1 LocalStorageが残っていてS-01に遷移しない
...
### 7.5 BUG-VALIDATOR（ST-06-07）後のクリーンアップ忘れ
